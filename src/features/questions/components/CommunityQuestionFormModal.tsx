...
import { useCreateCommunityQuestion } from "@/features/questions/hooks";
...
type CommunityQuestionFormModalProps = {
  isOpen: boolean;
  onClose: () => void;
  communityName: string;
};

const CommunityQuestionFormModal = ({
  isOpen,
  onClose,
  communityName,
}: CommunityQuestionFormModalProps) => {
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [isUploadingImage, setIsUploadingImage] = useState(false);

  const { data: session, isPending: isSessionLoading } =
    authClient.useSession();
  const { mutate, isPending } = useCreateCommunityQuestion();
  ...
  const onSubmit = async (data: CreateQuestionType) => {
    ...
    mutate(
      {
        communityName,
        payload: { ...data, authorId: session.user.id, image: imageUrl },
      },
      {
        onSuccess: () => {
          reset();
...
export default CommunityQuestionFormModal;
